...
The `llm()` function creates an LLM instance for chat and completion with Large Language Models.
...
The returned instance provides the following interface:
...
<Aside type="note" title="Static Capabilities">
...
`llm()` core checks capabilities before using features:
...
- `structure` provided but `capabilities.structuredOutput` is `false` → MUST throw `INVALID_REQUEST`
...
See [Error Handling](/spec/errors/) for full details on error codes, HTTP status mapping, and retry strategies.
