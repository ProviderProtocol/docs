...
The persistence middleware automatically loads and saves threads around each request:
...
const adapter = new PersistenceAdapter({
  id: 'conversation-123',
  load: async (id) => loadThreadFromDatabase(id),
...
// History is loaded automatically before each request
// and saved automatically after completion
const turn = await claude.generate('Hello!');
```

### Manual Serialization

For more control, use `toJSON()` and `fromJSON()`:
