...
interface TokenUsage {
  ...
  /** Tokens read from cache (cache hits). Returns 0 for providers that don't support cache metrics. */
  cacheReadTokens: number;

  /** Tokens written to cache. Only Anthropic reports this; returns 0 for other providers. */
