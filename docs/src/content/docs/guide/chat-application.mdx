...
const tools = [
  {
    ...
    async run({ location }) {
      ...
      const result = { location, temperature: `${temp}°F`, conditions };
...
async function main() {
  ...
  console.log('💬 Chat with GPT-4o (type "exit" to quit, "clear" to reset)\n');
  ...
  const prompt = () => {
    rl.question("You: ", async (input) => {
      ...
      if (trimmed.toLowerCase() === "clear") {
        ...
        console.log("\n🗑️  Conversation cleared. Restart to begin fresh.\n");
        rl.close();
        return;
      }
...
💬 Chat with GPT-4o (type "exit" to quit, "clear" to reset)

You: whats the weather in sheboygan milwaukee and greenbay

   🔧 Fetching weather for Sheboygan...
   ✅ Result: {"location":"Sheboygan","temperature":"66°F","conditions":"cloudy"}

   🔧 Fetching weather for Milwaukee...
   ✅ Result: {"location":"Milwaukee","temperature":"64°F","conditions":"partly cloudy"}

   🔧 Fetching weather for Green Bay...
   ✅ Result: {"location":"Green Bay","temperature":"66°F","conditions":"sunny"}
Assistant: Here is the current weather:

- **Sheboygan:** 66°F, cloudy
- **Milwaukee:** 64°F, partly cloudy
- **Green Bay:** 66°F, sunny
```
...
```json
{
  ...
  "messages": [
    ...
    },
    {
      ...
      "type": "tool_result",
      "results": [
        { "toolCallId": "call_9i0v...", "result": "{\"location\":\"Sheboygan\",\"temperature\":\"66°F\",\"conditions\":\"cloudy\"}" },
        { "toolCallId": "call_J60P...", "result": "{\"location\":\"Milwaukee\",\"temperature\":\"64°F\",\"conditions\":\"partly cloudy\"}" },
        { "toolCallId": "call_xh24...", "result": "{\"location\":\"Green Bay\",\"temperature\":\"66°F\",\"conditions\":\"sunny\"}" }
      ],
...
- [Thread API Reference](/api/types/thread)
- [Tool Types](/api/types/tool)
- [llm() Function](/api/core/llm)
